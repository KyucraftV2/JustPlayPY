    ...
    def attaquer(self,adv):
        ...
        for i in range(len(Map.obstacle_dic)):
            ...
            if abs(player1[i]) == abs(player2[i]):
                ...
                if  ((player1[i]<0) and (player2[i]>0))or ((player1[i]>0) and (player2[i]<0) or(((player1[i]==0) and (player2[i]==0)))):
                    touché_caillou = True
                    ...
                    print("obstacle derriere la cible")
                    pass
...
class Game:
    ...
    def vrai_obstacle(self):
        for i in range(len(Map.obstacle_dic)):
            if Map.mapa[Map.prenom[0]] == Map.obstacle_dic[i]:
                return True
        return False
